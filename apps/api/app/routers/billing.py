...
def get_billing(request: Request) -> BillingService:
    service = request.app.state.billing
    if not isinstance(service, BillingService):
        raise RuntimeError("Billing service is not initialized")
    return service


def _user_or_401(user: UserRecord | None) -> UserRecord:
    if not user:
        raise HTTPException(status_code=401, detail={"code": "unauthorized", "detail": "Not authenticated"})
    return user


def _billing_http(exc: BillingError) -> NoReturn:
    raise HTTPException(status_code=exc.http_status, detail={"code": exc.code, "detail": exc.message})


def _consent_http(exc: AuthConsentError) -> NoReturn:
    raise HTTPException(status_code=exc.http_status, detail={"code": exc.code, "detail": exc.message})


@router.get("/plans")
async def list_plans(service: BillingService = Depends(get_billing)) -> list[dict[str, Any]]:
    return service.list_plans()


@router.get("/me")
async def my_billing(
    service: BillingService = Depends(get_billing), user: UserRecord | None = Depends(current_user)
) -> dict[str, Any]:
