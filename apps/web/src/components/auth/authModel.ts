...
export const emptyAccepts = (): Accepts => ({
  terms_of_use: false,
  offer: false,
...
export function validateCredentials(input: Credentials): FieldError[] {
  ...
  if (input.displayName.trim().length < 2 || input.displayName.trim().length > 80)
    errors.push({ id: 'reg-name', message: 'Укажите имя пользователя: от 2 до 80 символов.' });
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input.email.trim()))
    errors.push({ id: 'reg-email', message: 'Проверьте email, например name@example.com.' });
  if (
    input.password.length < 10 ||
    input.password.length > 128 ||
    !/[A-Za-zА-Яа-яЁё]/.test(input.password) ||
    !/\d/.test(input.password)
  )
    errors.push({ id: 'reg-password', message: 'Нужны 10–128 символов, буква и цифра.' });
  return errors;
}
export function parseLegalItems(value: unknown): LegalItem[] {
  ...
  for (const item of value) {
    if (
      ...
      !/^[a-fA-F0-9]{64}$/.test(item.content_hash ?? '') ||
      seen.has(item.consent_id)
    )
      throw new Error('invalid_legal_documents');
