...
const loadLegal = (signal: AbortSignal) =>
  apiFetch<unknown>('/legal/documents/active', { signal, retries: 0 });
...
function errorMessage(error: unknown): string {
  if (error instanceof ApiError) {
    ...
    if (error.status === 400 || error.status === 422)
      return 'Не удалось принять данные. Проверьте поля и актуальность согласий.';
    if (error.code === 'network' || error.code === 'timeout')
      return 'Не удалось подтвердить результат запроса. Проверьте подключение и почту перед повторной отправкой.';
  }
